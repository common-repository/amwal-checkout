...
abstract class Field {
	...
	protected function maybe_render_timestamp() {
		...
		$amwalwc_timestamps = \get_option( AMWALWC_SETTINGS_TIMESTAMPS, array() );

		if ( empty( $amwalwc_timestamps ) || empty( $amwalwc_timestamps[ $this->args['name'] ] ) ) {
			return;
		}

		$timestamp           = $amwalwc_timestamps[ $this->args['name'] ];
		...
		?>
		<p class="description amwalwc-setting-timestamp"><small><em><?php esc_html_e( 'Last updated', 'amwal' ); ?>: <?php echo esc_html( $formatted_timestamp ); ?></em></small></p>
		<?php
	}
}
